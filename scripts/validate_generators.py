...
def validate_gm_mail() -> None:
    ...
    assert list(output["dest_email"].astype(str).head(1)) == ["pipe5550@gmail.com"], "GM Mail no antepuso semilla"
    assert list(output["OPERACION"].astype(str)) == ["1234", "OP1", "OP4"], "GM Mail no conservo orden/dedupe esperado"
    ...
    assert output.loc[1, "FECHA_VENCIMIENTO_CUOTA"] == "05-02-2026", "GM Mail no formateo vencimiento"
    assert output.loc[0, "FECHA_ARCHIVO"] == "17-06-2026", "GM Mail no aplico fecha archivo"
    assert output.loc[2, "OPERACION"] == "OP4", "GM Mail no conserva operacion sin match"
    assert output.loc[2, "NOMBRE"] == "", "GM Mail operacion sin match debe quedar sin datos SQL"
    ...
    assert len(crm) == 2, "GM Mail CRM debe excluir operaciones sin datos"
    ...
    assert crm.loc[0, "NRO_DOCUMENTO"] == "1234", "GM Mail CRM no conserva operacion semilla"
    assert crm.loc[1, "NRO_DOCUMENTO"] == "OP1", "GM Mail CRM no conserva operacion"
    assert crm.loc[1, "CORREO"] == "primero@example.com", "GM Mail CRM no conserva correo"
    ...
    assert list(extension["dest_email"].astype(str).head(1)) == ["pipe5550@gmail.com"], "GM Extension no mantiene semilla"
    print("GM_MAIL_OK")
