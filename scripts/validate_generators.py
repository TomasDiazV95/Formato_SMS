...
def validate_gm_mail() -> None:
    ...
    assert len(crm) == 3, "GM Mail CRM debe excluir operaciones sin datos"
    ...
    assert crm.loc[1, "NRO_DOCUMENTO"] == "1234", "GM Mail CRM no conserva segunda semilla"
    assert crm.loc[2, "NRO_DOCUMENTO"] == "OP1", "GM Mail CRM no conserva operacion"
    assert crm.loc[2, "CORREO"] == "primero@example.com", "GM Mail CRM no conserva correo"
